    ...
    <div className="card soft overflow-hidden" style={{ borderColor: `${BRASS}55` }}>
      ...
      <div className="p-5 flex gap-4 items-start">
        <div
          ...
          style={{ width: 64, height: 64, borderRadius: 12, background: a.avatarColor, color: "#fff", fontSize: 22, border: `1px solid ${LINE}` }}
        >
          {a.name.split(" ").map((w) => w[0]).join("")}
        </div>
        <div className="min-w-0">
          <p className="disp font-bold text-xl leading-tight">{a.name}<Tick /></p>
          <p className="text-sm mt-0.5" style={{ color: MUTED }}>{a.level} {a.trade}</p>
          <p className="text-sm mt-0.5" style={{ color: MUTED }}>{a.area}</p>
